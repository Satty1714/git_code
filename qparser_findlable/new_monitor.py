...
def OpenQRulerDB(qruler_file):
    ...
    dirname_temp = r"C:\SUNYAN\20181122_FindLable\QParser\Qparser_FindLable"
...
def printf(everything="", SIGN=True):
    if print_SIGN and SIGN:
        py_file = sys._getframe(0).f_back.f_code.co_filename.split("\\")[-1]
        py_file = py_file.ljust(15, " ")
        func_name = sys._getframe(0).f_back.f_code.co_name
        ...
        lines = str(sys._getframe(0).f_back.f_lineno).ljust(5, " ")
        if everything == "":
            print("{}{} {} {} {}".format(py_file, GetTime(), func_name, lines, "*"*10))
        else:
            print("{}{} {} {} {}".format(py_file, GetTime(), func_name, lines, everything))

def ReadCaseID(file_name):
    dirname, _ = os.path.split(os.path.abspath(__file__))
    if not os.path.isfile("{}\\{}".format(dirname, file_name)):
        return []
...
def SaveCaseID(file_name, case_id_list):
    dirname, _ = os.path.split(os.path.abspath(__file__))
    with open("{}\\{}".format(dirname, file_name), 'a') as f:
...
# 获得文本内容 测试通过
def GetText_Xpath_backup(browser, xpath, sign=True, count=0, text=""):
    while (sign):
        try:
            count += 0.5
...
def GetBestNewCaseIdInfo(browser):
    ...
    print(id_number_time_list)
    return id_number_time_list


def GetLable_Head(description_str,keyword):
    lable, index = "", 0
...
def MatchingTemplate_backup(keywords_list,description_str):
    printt(keywords_list)
    log.logger.info(keywords_list)
    description_list = description_str.split("\n")
    lable, template_sign = "", {}
    for keyword in keywords_list:
        # pattern = keyword.strip().replace(" ", ".*?")
        ...
        for description in description_list:
            ...
            if not match:
                ...
                lable = GetLable_Head(description_str, keywords_list[1])
                break
    printt(lable)
...
def MatchingTemplate(keywords_list,description_str):
    ...
    def FindLabel_t(description_list):
        ...
        if len(label_label_list) == 2:
            ...
            if count == 2:
                printt(count)
                log.logger.info("list中都是label头")
                return "" #都是头没有尾
            if count == 1:
                if count_index == 0:
                    log.logger.info("=".join(label_label_list))
            ...
            if count == 0:
                printt(count)
                log.logger.info("list中都是label尾，也就是重复label")
                return "" #都是尾没有头
...
def GetCommnets(browser,case_id):
    comments_list = []
    try:
        #//*[@id="5003A00000t0SVZ_RelatedCommentsList_body"]/table/tbody
        #//*[@id="5003A00000t0SVZ_RelatedCommentsList_body"]/table/tbody
        printt('//*[@id="{}_RelatedCommentsList_body"]/table/tbody'.format(case_id))
        rows1, cols1 = GetTableRowsAndCols_xpath(browser, '//*[@id="{}_RelatedCommentsList_body"]/table/tbody'.format(case_id), "tr", 'th')
    except:
        return []
    ...
    for row in range(2, rows1+1):
        ...
        # description_str = GetText_Xpath(browser, comments_xpath)
        try:
            comments_xpath = '//*[@id="{}_RelatedCommentsList_body"]/table/tbody/tr[{}]/td[2]/div'.format(case_id, row)
            printt(comments_xpath)
...
def FindComments_BACKUP2(firstcomment_reminder_str,comments_list):
    template_list = []
...
def GetUrl_Xpath_backup(browser, xpath, sign=True, count=0, text=""):
    while (sign):
        try:
            count += 0.5
            resolved_during_customer_browser = browser.find_element_by_xpath(xpath)
...
def UpdateCookies_backup(cookies_path):
    count, bestnew_cookies = 2, ""
    while (count):
        if bestnew_cookies == "":
            ...
            count = 0
    if bestnew_cookies == "":
        printt("Unable to get the latest cookies")
        # log.logger.error("Unable to get the latest cookies")
        log.logger.info("Unable to get the latest cookies")
        exit()
...
def Call_Lattice_QParser(lable_value,case_number,name_link, qruler_db):
    # printt("Call_Lattice_QParser")
    dirname, _ = os.path.split(os.path.abspath(__file__))
    qparser_path, _ = os.path.split(dirname)
...
def GetSubjectLable_backup(lable_str):
    '''
    ...
    for index in range(counts):
        ...
        if type(testcase_tail) != "str":
            testcase_tail = '{}'.format((testcase_tail))
        lable_list.append("{};{}".format(testcase_tail, testspec[index]))
    printt(lable_list)
    log.logger.info(lable_list)
    printt(lable_list[::-1])
    log.logger.info(lable_list[::-1])
    find_lable = []
    for lable in lable_list[::-1]:
        lable_tail = lable.split(";")[0]
        lable_head = lable.split(";")[1]
        str_list = lable_str.split("\n")
    ...
    if not len(find_lable):
        return ""
    printt(find_lable)
    log.logger.info(find_lable)
    printt(lable_str)
    log.logger.info(lable_str)
    sign_count = 1
    #判断是否为多个lable值
...
def add_interoperability_backup(browser,lable_str):
    ...
    if lable_str not in text:
        ...
        if len(text) == 0 or text == " ":
            ...
            text = "{},{}".format(text,lable_str)
        browser.find_element_by_xpath('//*[@id="pg:frm:blk:productData:interOperabilityIOTDeviceId"]').send_keys(text)
...
def UpdateSign_backup(template,comment):
    template_list = []
    for tl in template.split("\n"):
        if template in [" ", ""]:
            continue
        # print("template:({})".format(tl))
        template_list.append(tl)
    for tl in template_list:
...
def Main():
    ...
    dirname_T, _ = os.path.split(os.path.abspath(__file__))
    if os.path.isfile("{}\\log.txt".format(dirname_T)):
        os.remove("{}\\log.txt".format(dirname_T))
    ...
    # need_id_list = ['5003A00000t0SVZQA2']
    # need_id_list = ['5003A00000t0SVZ']
    # need_id_list = ["5003A00000t0SXa"]
    for template_case_id in need_id_list:
        ...
        for name_link in url_list:
            ...
            if log_path == "":
                ...
                continue
            _, log_name_t = os.path.splitext(log_path)
            if log_name_t == ".html":
                printt("continue, log_path:({})".format(log_path))
                log.logger.info("continue, log_path:({})".format(log_path))
                continue
            ...
            if len(output_list) == 0 or output_list[0] == "phone_log" or output_list[0] == "size:0" or output_list[0] == "not exits":
                comment_ownercheck_update = qruler_db['comment_ownercheck'].iloc[0].strip('\n')
                comments_list1 = comments_list[:]
                # comments_list1.append("{}\n{}".format(log_path_name,comment_ownercheck_update))
                # update_log.logger.info(comments_list1)
                # update_log.logger.info(comment_ownercheck_update)
    ...
    with open("{}\\log.txt".format(dirname_T), "w") as f:
