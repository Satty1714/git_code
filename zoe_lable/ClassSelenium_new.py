	...
	def GetText_Xpath(self, xpath, sign=True):
		...
		while (sign):
			try:
				...
				print("resolved_during_customer_browser.text:("+resolved_during_customer_browser.text+")")
				sign = False
			except:
				if count == 10:
					sign = False
...
#add function sunyan 20171226 begiin
#单击操作 测试通过
...
#根据class直接获得行数
def GetTableAllRows_ClassName(browser,class_name, label):
    '''根据class直接获得行数'''
    #label = 'tr'
...
# 获得文本内容 测试通过
def GetText_Xpath(browser, xpath, sign=True, count = 0):
    ...
    while (sign):
        try:
            ...
            if count == 10:
                ...
                sleep(0.5)
    return text
#add function sunyan 20171226 end
'''
se_ce = Selenium_CE()
